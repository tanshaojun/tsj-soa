...
public class JsonMapper extends ObjectMapper {
    ...
    /**
     * 统一判断请求58集装箱返回code code为0时返回对象 否则返回null
     *
     */
    public <T> T parseJsonRes(String jsonString, Class<T> clazz) {
        if (StringUtils.isEmpty(jsonString)) {
            return null;
        }
        try {
            ...
            if ("0".equals(code)) {
                return this.readValue(dataObject.toJSONString(), clazz);
            }
        } catch (IOException e) {
            logger.warn("parse json string error:" + jsonString, e);
            return null;
     ...
     *
     */
    public String getResCode(String jsonString) {
        if (StringUtils.isEmpty(jsonString)) {
            return null;
        }
        JSONObject jsonObject = JSON.parseObject(jsonString);
